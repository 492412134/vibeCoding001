...
public class PaymentAggregator {
    ...
    public void submitPayment(PaymentRequest request) {
        ...
        if (request.getOrderType() == null) {
            request.setOrderType(com.example.payment.payment.enums.OrderType.NORMAL);
        }

        // 1. 生成雪花ID（如果未设置）
        if (request.getSnowflakeId() == null) {
            // 优先使用传入的政策类型，如果没有则使用默认值（测试场景）
            PolicyType policyType;
            if (request.getPolicyType() != null && !request.getPolicyType().isEmpty()) {
                policyType = PolicyType.fromName(request.getPolicyType());
            } else {
                // 测试场景：默认使用common政策类型
                policyType = PolicyType.COMMON;
            }
            // 优先使用传入的政策编号，如果没有则随机生成（测试场景）
            long policyCode;
            if (request.getPolicyCode() != null && request.getPolicyCode() > 0) {
                policyCode = request.getPolicyCode();
            } else {
                // 测试场景：随机生成政策编号（1-32）
                policyCode = ThreadLocalRandom.current().nextLong(1, 32);
            }
